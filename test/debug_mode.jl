@testset "debug_mode" begin
    ...
    @testset "forward debug mode" begin
        @testset "argument checking" begin
            # Mismatched primal types must fail before entering the rule to avoid a segfault.
            # The registry builds and calls with the same types, so cannot express this case.
            f = x -> 5x
