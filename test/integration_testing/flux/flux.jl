...
#
#   1. CORRECTNESS — cuDNN / NNlib primitives (BatchNorm, softmax, …) are C++ library
#      calls that receive raw Float32/Float64 pointers; they never see NDual-element
#      arrays, so the GPU compiler fails to generate a kernel.
...
const TEST_MODELS = [
    (_gpu_enabled, Dense(2 => 4), randn(Float32, 2), "Dense"),
    (
        _gpu_enabled,
    ...
    (_gpu_enabled, Conv((3, 3), 2 => 3), randn(Float32, 3, 3, 2, 1), "Conv"),
    (
        _gpu_enabled,
    ...
    (_gpu_enabled, Maxout(() -> Dense(5 => 4, tanh), 3), randn(Float32, 5, 1), "Maxout"),
    (
        _gpu_disabled,
    ...
    (_gpu_enabled, Flux.Bilinear((2, 2) => 3), randn(Float32, 2, 1), "Bilinear"),
    (
        _gpu_enabled,
    ...
    (_gpu_enabled, LayerNorm(2), randn(Float32, 2, 10), "LayerNorm"),
    (_gpu_disabled, BatchNorm(2), randn(Float32, 2, 10), "BatchNorm"),  # GPU kernel receives non-bitstype: CPU Matrix{Float32} tangent mixed into CuArray broadcast
    (
        _gpu_disabled,
        first ∘ MultiHeadAttention(16),
        randn32(16, 20, 2),
        "MultiHeadAttention",
    ),  # Illegal conversion of CUDA.DeviceMemory to Ptr{Float32} in attention path
    (_gpu_enabled, RNN(3 => 2), randn(Float32, 3, 2), "RNN"),
    (_gpu_enabled, LSTM(3 => 5), randn(Float32, 3, 2), "LSTM"),
    (_gpu_enabled, GRU(3 => 5), randn(Float32, 3, 10), "GRU"),
    (
        _gpu_enabled,
