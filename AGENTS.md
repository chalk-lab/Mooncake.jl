...
- The return type of `frule!!` should match `dual_type(Val(N), typeof(primal_result))`. Use `zero_dual(width, result)` to get the canonical shape. Wrapping an NDual-bearing result in `Dual(..., NoTangent())` is incorrect — it forces every downstream rule to grow a parallel Dual-wrapped overload as the polarity flip propagates.
...
- `NDual` (the chunked width-N representation) only appears as: (a) a bare scalar `NDual{T,N}` from `IEEEFloat` primals, (b) inside `Complex{NDual{T,N}}`, (c) as an element of `Array` / `Memory` / `MemoryRef`, or (d) as an element of a `Tuple` (concrete `Tuple` types lift element-wise). It does not appear inside struct fields or as the tangent of a non-IEEEFloat primal — those go through `Dual{S, NTangent{...}}` instead. Rule signatures and helper dispatch should match these shapes only.
- Keep NDual-specific rules in `src/rules/rules_via_nfwd.jl`; restrict NDual-based `dual_type` shapes to `IEEEFloat`, `Complex{<:IEEEFloat}`, arrays of those scalar types, and tuples of those scalar/container shapes.
- **`tangent_type` is recursively coherent everywhere.** For any primal type `P`, if `tangent_type(P) = T`, then for every accessible field/element/component of a `P` value, its tangent has type `tangent_type(field_type)`. Struct lifts use `Tangent{NamedTuple{names, Tuple{(tangent_type(field_type_i))...}}}` (immutable) or `MutableTangent{...}` (mutable); `tangent_type(Array{T,D}) === Array{tangent_type(T),D}` (or `Array{T,D}` for IEEEFloat); `tangent_type(Tuple{T_i...}) === Tuple{(tangent_type(T_i))...}`; specialised overrides preserve this recursion by construction. Reverse-mode rules (`rrule!!`, `zero_tangent`, `randn_tangent`, `fdata`/`rdata`) rely on this invariant — any new `tangent_type` overload must respect it.
- **`dual_type` is NOT necessarily recursively coherent with `tangent_type`.** The canonical V (forward-mode lifted slot type) can be structurally different from `tangent_type(P)` — e.g., `dual_type(Val(1), Vector{Float64}) === Vector{NDual{Float64,1}}` (NDual-element form, single allocation) while `tangent_type(Vector{Float64}) === Vector{Float64}` (parallel storage). This divergence is intentional for performance and IR-emit predictability. Where the structural lift fires (Tuple, NamedTuple, immutable struct with `_uses_structural_dual_type`), `dual_type` does recurse element-wise; for explicit overrides (NDual-element arrays/wrappers, parallel-Dual wrapper-exceptions), the override defines a new V shape that may not mirror `tangent_type` at all. Forward-mode rules dispatching on canonical V should match the V shape directly (`AbstractMatrix{NDual{T,N}}`, `Dual{<:Wrapper, <:Tangent}`, etc.), never assume `V === tangent_type(P)`.
- `Lifted{P, N, V}` is the forward-mode slot wrapper, parallel to `CoDual{Tx, Tdx}`. For concrete runtime values only, `P` is concrete and `V === dual_type(Val(N), P)`. Abstract slots use broad, width-preserving annotations such as `lifted_type(Val(N), abstract_P)`; runtime values should be concrete wrappers.
- Construct rule outputs via `Lifted{P, N}(primal, tangent)`, not direct `Dual(...)` or `NDual{T, N}(...)` calls. Concrete `P` delegates to the inner type's constructors; abstract/nonconcrete `P` must sharpen through `typeof(primal)` and return a value compatible with the abstract slot, never `Any(primal, tangent)` or normal exact abstract wrappers.
- **Do not branch on inner-V shape inside Lifted-typed rule bodies.** Under the concrete runtime invariant, `dual_type(Val(N), P)` determines `V`; rule bodies should `_unlift`, operate on the bare canonical `V`, and construct `Lifted{P_out, N}(...)`.
- `Lifted` wraps once at the top level, including for concrete `Tuple` and `NamedTuple` primals. The inner `V` contains bare inner duals such as `NDual`, `Complex{NDual}`, `Array{NDual}`, `Dual`, or tuples of those; it never contains nested `Lifted`.
- In Mooncake's generated IR, never annotate a join value as `Lifted{Union{A,B}, ...}` when the actual runtime values flowing into that slot are `Lifted{A, ...}` and `Lifted{B, ...}`. Because `Lifted` is parametric and invariant, `Lifted{A, ...}` is not a subtype of `Lifted{Union{A,B}, ...}`. Use a sound join type such as `Union{Lifted{A, ...}, Lifted{B, ...}}`, a suitably broad `UnionAll`, or a representation that avoids wrapping the join in an invariant parametric type; otherwise downstream `PiNode`s and OpaqueClosure compilation may trust an invalid type fact and lower the impossible path to an `unreachable` trap.
...
- Forward mode currently produces silently-wrong tangents for user-defined **mutable** structs whose `Array{<:IEEEFloat}` fields are mutated in place (the `lgetfield` → `_wrap_rule_result` chain canonicalises the field into a fresh `Vector{NDual}` that is no longer aliased to the struct's storage; see `docs/src/known_limitations.md`). When you hit this, write a per-function `@is_primitive` rule with both a `Lifted`-typed source-of-truth body (reaching into `_unlift(s)` → `tangent(inner_s).lanes` to write each lane's `MutableTangent.fields.<array_field>` by hand) and a bare-`Dual` delegator that uses the 3-param `Lifted{P,1,V}(v)` ctor to avoid re-canonicalising. Reverse mode is unaffected.
