...
- Chunked-forward allocation: a width-`N` `frule!!` runs per lane, so a temp inside the lane loop costs `O(N)`. Hoist scratch above the loop (refill per lane via `copyto!` + in-place op), compute lane-independent work once, and prefer direct `BLAS.trsm!`/`trmm!`/`gemm!` over wrapped LinearAlgebra `!` ops (which allocate internally). Verify `@allocated` at width 1 vs 8.
- Cross-call allocation: a derived rule caches compiled code and (reverse) the self-resetting pullback `Stack`s, but re-`similar`s the working buffers on every call. A rule opts into cross-call reuse by drawing workspace from the process-global `ScratchPool` via `buffered_similar!(proto)` (a pooled `similar` for dense isbits arrays) instead of `similar`, filled by an in-place op — it stays an ordinary `frule!!`/`rrule!!`, no per-rule instance or `build_primitive_*` override. The pool is reset once per top-level AD call (`__value_and_gradient!!` / `__value_and_pullback!!` / forward `value_and_derivative!!` wrap their bodies in `with_scratch_pool`, under an outer lock that serializes top-level AD calls), so slots are reused across calls but not within one. There is deliberately **no per-call forward reuse**: each `buffered_similar!` pushes a fresh slot and the pool grows across a call (a forward loop holds one buffer per iteration), reclaimed only at the next call's reset; reverse rrules likewise just push (the whole forward sweep precedes the whole reverse sweep, so every pullback-captured buffer is live at the peak regardless). An inner lock makes the push atomic, so worker tasks inside a differentiated `Threads.@threads` region each get a distinct slot (top-level AD calls are already serialized by the outer lock). The pool doesn't touch allocations inside opaque calls (`BLAS`/`LAPACK` internals), from non-in-place ops (`A*B`→`mul!`), or non-buffer / composite-AD-value (`CoDual`/`Lifted`/`NDualArray`) allocations — zero-alloc reuse needs every buffer pooled AND every op in-place. A pooled buffer is reused memory, not a freshly-allocated tracked dual, so a buffered reverse restore-copy does not compose with forward-over-reverse (HVP/Hessian) — it fails loudly (a `:memmove` ccall error), never silently. A reverse rule whose restore-copy must survive nested AD should keep `Base.@noinline copy` (dispatchable to the `copy` `frule!!`, as `trtrs!` does) instead of pooling it.
