...
- Perturbation confusion / CUDA higher-order limitation: GPU kernels are foreign calls with no Julia IR, so the CUDA extension differentiates elementwise/reduction ops (`sum(f, x)`, broadcasts) by running the mapped function on `Nfwd.NDual` (forward-mode) numbers inside the kernel — in both forward and reverse rules. `NDual` is untagged and cannot nest (`NDual{NDual}` is not permitted and raises an error), so forward-over-reverse (HVP/Hessian) through these rules would collide the two perturbation levels (perturbation confusion). These paths error loudly (never silently wrong); HVP/Hessian for non-elementwise ops (`sum(x)`, `dot`, BLAS) works. The intended fix is a nested second-order dual (a `HyperDual`).
