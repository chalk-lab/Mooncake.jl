...
- **Both `tangent_type` and `dual_type` must be recursively coherent.** For any primal type `P`, if `tangent_type(P) = T`, then for every accessible field/element/component of a `P` value, its tangent has type `tangent_type(field_type)`. Symmetrically, if `dual_type(Val(N), P) = V`, then every accessible field/element of `V` must have type `dual_type(Val(N), field_type)`. Both invariants apply to:
  ...
  Reverse-mode rules (`rrule!!`, `zero_tangent`, `randn_tangent`, `fdata`/`rdata`) and forward-mode rules (`frule!!`, the Lifted-V invariant `V === dual_type(Val(N), P)`) both rely on these invariants. **Any new `tangent_type` or `dual_type` overload must respect the recursive contract** — a non-coherent `dual_type` (e.g., one where a field of `V` doesn't equal `dual_type(Val(N), field_type)`) breaks `lgetfield` chains and causes silent-corruption bugs in forward AD on mutable structs with array fields (see `docs/src/known_limitations.md`). For mutable structs whose Array fields' canonical V is the NDual-element form, the coherent shape is `SplitDual{NamedTuple{names, Tuple{(dual_type(Val(N), field_type_i))...}}}` — a single mutable NamedTuple holding canonical Vs per field, with no separate primal-half storage (primal values live inside each NDual's `.value`).
