...
- `NDual` (the chunked width-N representation) appears inside the canonical forward representation `V = dual_type(Val(N), P)`, not inside the user's primal struct object. Direct scalar/container shapes include: (a) a bare scalar `NDual{T,N}` from `IEEEFloat` primals, (b) `Complex{NDual{T,N}}`, (c) elements of `Array` / `Memory` / `MemoryRef`, and (d) elements of concrete `Tuple` / `NamedTuple` types. For structural primal types, `NDual` may appear recursively inside the structural inner `V` (`NamedTuple` or `SplitDual{<:NamedTuple}`) for fields whose own `dual_type` contains `NDual`. It should not be added as fields of the original primal struct, and rule signatures/helper dispatch should match only shapes returned by `dual_type`.
- Keep NDual-specific rules in `src/rules/rules_via_nfwd.jl`; restrict NDual-based direct scalar/container `dual_type` shapes to `IEEEFloat`, `Complex{<:IEEEFloat}`, arrays/memory of those scalar types, tuples/named-tuples of those scalar/container shapes, and structural `NamedTuple` / `SplitDual` forms produced recursively from those fields.
...
- **Do not branch on inner-V shape inside Lifted-typed rule bodies.** Under the concrete runtime invariant, `dual_type(Val(N), P)` determines `V`; rule bodies should prefer the public accessor surface (`primal(slot)`, `tangent(slot)`, and `tangent(slot, lane)`) and construct outputs with `Lifted{P_out, N}(...)`. Use `_unlift` / `_lift` only for centralized compatibility or boundary code, not as repeated per-primitive rule scaffolding.
- `Lifted` wraps once at the top level, including for concrete `Tuple` and `NamedTuple` primals. The inner `V` contains bare inner duals such as `NDual`, `Complex{NDual}`, `Array{NDual}`, `Dual`, tuples/named-tuples of those, or `SplitDual` for mutable structural shapes; it never contains nested `Lifted`.
