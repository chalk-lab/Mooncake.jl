...
- A subtle second-order form of the same world-age trap: even when sub-function calls are placed in the returned expression, the @generated body's *expansion-time throw* (e.g. `return error(...)` in the meta-function body) bakes the throw into the caller's compiled IR. The `@foldable` annotation then lets the optimizer cache that throw and Julia does not invalidate the cache when a later, more-specific overload is added — compiled OpaqueClosure bodies (e.g. from `build_frule`) keep their cached throw and can't be dislodged by a new overload in any later world. The principled mitigation, used by both `tangent_type` and `fdata_type` for their primitive-type fallback, is to return a *runtime* error expression: `msg = "..."; return :(error($msg))`. The throw then fires only if dispatch actually reaches the @generated fallback at runtime, which lets a more-specific extension overload override it. Always use the deferred-error pattern when adding a new @generated `tangent_type`-like function.
...
- `frule!!` must return the canonical `dual_type(Val(N), typeof(primal_result))` shape. Use `zero_dual(Val(N), result)` for zero derivatives. Do not wrap NDual-bearing results in `Dual(..., NoTangent())`; that forces downstream `Dual`-wrapped overloads.
- The legacy two-field `Dual{P, T}` (with `primal::P, tangent::T`) is retained as the public-compatibility boundary for ordinary width-1 forward use; rule signatures and dispatch helpers must not assume that array elements are themselves `Dual`. `Array{<:Dual}` and similar element-wise wrappings are not part of the `dual_type` protocol and should not appear in normal AD flow — do not add rule overloads matching them.
- `NDual` appears only inside the canonical forward representation `V = dual_type(Val(N), P)`, never as fields added to the user's primal struct. Direct NDual-bearing shapes are `IEEEFloat`, `Complex{<:IEEEFloat}`, `NDualArray{T,N,D,A}` for arrays/memory of those scalars (SoA wrapper, see §14 of the dual-types design note), tuples/named-tuples of those shapes, and the recursive structural lift `ImmutableDual{NamedTuple{...}}` (immutable struct) / `MutableDual{NamedTuple{...}}` (mutable struct). Keep NDual-specific rules in `src/rules/rules_via_nfwd.jl`, and match only shapes returned by `dual_type`.
...
- **Both `tangent_type` and `dual_type` must be recursively coherent.** For every accessible field, element, or component of `P`, the tangent representation is `tangent_type(component_type)` and the forward representation is `dual_type(Val(N), component_type)`. This applies to:
  - Struct lifts: `tangent_type` produces `Tangent{NamedTuple{names, Tuple{(tangent_type(field_type_i))...}}}` (immutable) or `MutableTangent{...}` (mutable); `dual_type`'s structural lift mirrors this with per-field `dual_type(Val(N), field_type_i)` wrapped in `ImmutableDual{NamedTuple{...}}` (immutable) or `MutableDual{NamedTuple{...}}` (mutable).
  - Arrays: `tangent_type(Array{T,D}) === Array{tangent_type(T),D}` (or `Array{T,D}` for IEEEFloat); `dual_type(Val(N), Array{T,D}) === NDualArray{T, N, D, Array{T,D}}` — the SoA wrapper whose `primal::A` aliases user storage and whose `partials::NTuple{N, A}` is slot-local.
  ...
  Reverse rules (`rrule!!`, `zero_tangent`, `randn_tangent`, `fdata`/`rdata`) and forward rules (`frule!!`, `V === dual_type(Val(N), P)`) rely on this. A non-coherent `dual_type` breaks `lgetfield` chains and can silently corrupt forward AD on mutable structs with array fields (see `docs/src/known_limitations.md`). The forward-mode structural-lift wrappers `ImmutableDual` and `MutableDual` are single-field (each holds the recursive `NamedTuple` only); they mirror reverse-mode `Tangent` / `MutableTangent`. Slot-level primal lives in `Lifted{P, N, V}` (`primal::P`, `value::V`), not inside the V wrappers.
- `Lifted{P,N,V}` is the forward-mode slot wrapper, parallel to `CoDual{Tx,Tdx}`. It is a two-field struct (`primal::P, value::V`). For concrete runtime values, `P` is concrete and `V === dual_type(Val(N), P)`. Abstract slots use broad, width-preserving annotations such as `lifted_type(Val(N), abstract_P)`, while runtime values stay concrete. `primal(d::Lifted) = d.primal` is an O(1) field load at the slot level.
- Construct rule outputs with `Lifted{P,N}(primal, value)` or `Lifted{P,N}(primal, seed_tangent)`, not direct `Dual(...)` or `NDual{T,N}(...)` calls. Concrete `P` delegates to the inner construction path; abstract/nonconcrete `P` must sharpen through `typeof(primal)` and return a value compatible with the abstract slot, never `Any(primal, tangent)` or an exact abstract wrapper.
...
- `Lifted` wraps once at the top level, including for concrete `Tuple` and `NamedTuple` primals. The inner `V` contains bare inner duals (`NDual`, `Complex{NDual}`, `NDualArray`, `NDualMemoryRef`, tuple/named-tuple values of those, `ImmutableDual`, or `MutableDual`), never nested `Lifted`.
...
- The forward-mode type system has three sibling design notes:
  - `docs/src/developer_documentation/dual-types.md` — `NDual`, `NTangent`, `dual_type`, the structural-lift wrappers `ImmutableDual` / `MutableDual`, `NDualArray` / `NDualMemoryRef` (SoA array canonical V), `PossiblyUninitTangent` threading, the `MutableDualTangentView` proxy, and the forward/reverse aliasing asymmetry.
  - `docs/src/developer_documentation/lifted-types.md` — the `Lifted{P, N, V}` slot wrapper, three-layer type hierarchy (`tangent_type` / `dual_type` / `lifted_type`), public API surface, and rule-body convention.
  - `docs/src/developer_documentation/primal-mode.md` — primal-mode forward AD compiler design, `DerivedPrimal` / `LazyPrimal` rule objects, OpaqueClosure-based compilation, and the forward-over-reverse HVP path via `_GradClosure`.
