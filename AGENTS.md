...
- Performance parity with reverse: a single chunked (width-`N`) `frule!!` pass legitimately uses more memory than the reverse rule — it holds the `N` parallel partial buffers — but its runtime should be on par with a single `rrule!!` VJP. So the higher allocation is expected and not the bug; the runtime is the metric. If a `frule!!` pass is materially slower than the reverse rule, profile and fix (usual causes: per-lane primal recompute, unhoisted per-lane allocation, wrapped-LinearAlgebra where fused BLAS fits, an unreused factorization). The irreducible exceptions are scalar element-wise ops (`N` evaluations) and per-lane reductions.
