...
- Defining `tangent_type` / `fdata_type` / `dual_type` / `lifted_type` at runtime (via `Mooncake.eval`) or first-loading the method in a package extension is subject to a related trap: the `@foldable @generated` bodies that wrap these cache per-signature specializations, and Julia does not invalidate the cache when later, more-specific methods are added. Compiled OpaqueClosure bodies (e.g. from `build_frule`) can keep using the original @generated expansion even when a more-specific overload now exists. Prefer defining these overloads in `src/` so they precede any extension; if an overload must live in `ext/`, accept that already-compiled OC bodies may still reach the @generated fallback for primitive-type fields.
