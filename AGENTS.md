...
- `Lifted{P, N, V}` is the forward-mode slot wrapper (parallel to `CoDual{Tx, Tdx}` for reverse mode). `P` is the primal type, `N` is the width, and `V` is the inner dual shape — fully determined by `(P, N)` via the invariant `V === dual_type(Val(N), P)`. Rule signatures and helper dispatch should match `Lifted{P, N}` patterns directly; the inner `V` is invisible to rule bodies. Construct outputs via `Lifted{P, N}(primal, tangent)` (mirrors `CoDual(x, dx)`); never call `Dual(...)` or `NDual{T, N}(...)` directly in rule bodies — the inner-shape choice lives in the inner type's own constructor methods.
- **Do not branch on inner-V shape inside Lifted-typed rule bodies.** `dual_type(Val(N), P)` fully determines the inner V from the primal type at each width; runtime `isa Complex{<:NDual}` / `isa Dual{Complex, Complex}` checks are dead code under the canonical V invariant. Rule bodies should `_unlift` the input and operate on the bare canonical V directly, then construct a `Lifted{P_out, N}(...)` output. The `rules_via_nfwd.jl` scalar-math template (`Lifted{P_out, N}(f(_unlift(x)))`) is the reference pattern; see commit `3cb491801` for the principle applied to a `getfield`-style rule (13-line `isa`-branching body shrunk to 4 lines).
- `Lifted` is the top-level slot wrapper, like `CoDual`: every primal lifts to exactly one `Lifted{P, N, V}` at the slot boundary, including `Tuple` and `NamedTuple` primals. For a tuple primal `P = Tuple{T1, T2, ...}`, the lifted slot is `Lifted{P, N, Tuple{dual_type(Val(N), T1), dual_type(Val(N), T2), ...}}` — the *inner* `V` is a tuple of bare inner duals (NDual / Complex{NDual} / Array{NDual} / Dual / etc., per the AGENTS.md NDual-shapes rule above), not a tuple of `Lifted`. `Lifted` never nests inside another `Lifted`'s `V`: no `Lifted{..., Tuple{Lifted, ..., Lifted}}`, no `Lifted{..., Array{Lifted, ...}}`, etc. Element-wise lifting belongs to `tangent_type` (Layer 1) and `dual_type` (Layer 2); `lifted_type` (Layer 3) always wraps once at the top.
