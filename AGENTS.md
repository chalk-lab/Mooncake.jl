...
- `frule!!` must return the canonical `dual_type(Val(N), typeof(primal_result))` shape. Use `zero_dual(width, result)` for zero derivatives. Do not wrap NDual-bearing results in `Dual(..., NoTangent())`; that forces downstream parallel Dual-wrapped overloads.
...
- `NDual` appears only inside the canonical forward representation `V = dual_type(Val(N), P)`, never as fields added to the user's primal struct. Direct NDual-bearing shapes are `IEEEFloat`, `Complex{<:IEEEFloat}`, **top-level** arrays/memory of those scalars (not nested-array elements), tuples/named-tuples of those shapes, and recursive structural `NamedTuple` / `SplitDual` forms produced from those fields. Keep NDual-specific rules in `src/rules/rules_via_nfwd.jl`, and match only shapes returned by `dual_type`.
- **Canonical NDual element form is reserved for top-level arrays.** `Array{NDual{T,N}, D}` is the canonical V *only* when `T<:IEEEFloat` (or `Complex{<:IEEEFloat}`). Nested arrays (`Array{Array{IEEEFloat, K}, D}` such as `Vector{Vector{Float64}}`) fall through to the parallel-Dual fallback `Dual{P, NTangent{NTuple{N, P}}}`. The reason: element-wise NDual interleaving requires every element to be a defined NDual, but Julia's `Vector(undef, n)` semantics permit unassigned slots, which cannot be lifted element-wise. The same constraint applies to Tuple/NamedTuple recursion — element-wise descent stops at the first non-leaf-scalar container. SplitDual eligibility likewise requires a *top-level* Array-of-IEEEFloat field; a `Vector{Vector{Float64}}` field does not enable SplitDual.
...
- **Both `tangent_type` and `dual_type` must be recursively coherent.** For every accessible field, element, or component of `P`, the tangent representation is `tangent_type(component_type)` and the forward representation is `dual_type(Val(N), component_type)`. This applies to:
  ...
  - Arrays: `tangent_type(Array{T,D}) === Array{tangent_type(T),D}` (or `Array{T,D}` for IEEEFloat). For **top-level** `Array{T,D}` with `T<:IEEEFloat` (or `Complex{<:IEEEFloat}`), `dual_type` is the canonical element-wise NDual form `Array{NDual{T,N},D}`. For **nested** Arrays (e.g. `Vector{Vector{Float64}}`), there is no element-wise NDual recursion — the outer container falls through to `Dual{P, NTangent{NTuple{N, P}}}` because Julia's `Vector(undef, n)` permits unassigned slots that can't be lifted element-wise.
