...
- **Do not branch on inner-`V` shape inside Lifted-typed rule bodies.** `dual_type(Val(N), P)` determines `V`; use `primal(slot)`, `tangent(slot)`, and `tangent(slot, lane)` — a *lane* (the standard term) is one derivative slot in a width-`N` forward seed — then construct outputs with `Lifted{P_out,N}(...)`. Use `_unlift` / `_lift` only for centralized compatibility, boundary code, or operations that naturally return an already-canonical inner value; not as repeated per-primitive scaffolding.
