...
- A second-order form of the same world-age trap: even when sub-function calls are correctly placed in the returned expression, the `@foldable` annotation on the `@generated tangent_type` (and `fdata_type`) bodies lets Julia constant-fold the result at the call site's compile time. The folded result is then specialization-cached and Julia does not invalidate the cache when later, more-specific methods are added. Compiled OpaqueClosure bodies (e.g. from `build_frule`) can keep returning the original folded result even when a new overload now exists. Define new `tangent_type` / `fdata_type` overloads in `src/` whenever feasible so they precede any extension; if an overload must live in `ext/`, accept that OC bodies built before the extension loaded may still reach the @generated fallback for primitive-type fields.
