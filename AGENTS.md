...
- Don't recompute the primal: `f(tangent(x))` already runs the primal op and stores it in the result's `.value` (the inner-value invariant). Computing `f(primal(x))` separately for the output evaluates a non-trivial op (transcendental, BLAS, allocation) twice — ~1.5× slower per call for scalar math, the dominant cost in element-wise forward AD. Read the primal from the dual result instead: `dy = f(tangent(x)); Lifted{_typeof(y),N}(_nfwd_out_value(dy), dy)` (or `dy.value` for a scalar `NDual`), as `pow_fast` does.
- Exploit fused primitives: when one call yields both the value and the derivative factor, use it instead of separate ops. `sincos` shares the libm work between `sin`'s value (`s`) and its derivative (`c`), so the `NDual` `sin`/`cos`/`tan` overloads call `sincos(value)` once; `muladd`/`fma` fuse a multiply-add. Prefer the fused form so value and partials come from a single evaluation.
- An array `frule!!` must work on the `NDualArray`'s parallel arrays directly: `nda.primal` and each `nda.partials[k]` are plain `Array`s, so `sum`/BLAS/broadcast over them vectorizes. Reducing or iterating the whole `NDualArray` element-wise (`sum(tangent(x))`, `mapreduce`/`dot`/`for i; nda[i]`) builds one `NDual` per element and folds via the scalar `_ndual_mapreduce_impl` left-fold — no SIMD, ~5× slower (e.g. forward `sum` over a 1000-vector). Prefer `NDual{P,N}(reduce(nda.primal), ntuple(k -> reduce(nda.partials[k]), Val(N)))`.
