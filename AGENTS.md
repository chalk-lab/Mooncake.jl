...
- Batching the `N` per-lane calls into fewer wide ones pays only with a cost to amortize. **Shared-factor solves** (triangular/LU/Cholesky): stacking the `N` lane RHS into one wide solve reuses the factorization — wins on CPU *and* GPU (`trsm!`/`trtrs!` 1.8–4.7× for `n≥64`). **GPU** wins broadly (launch-bound): `CUDA.CUBLAS.gemm_batched!`/`gemv_batched!` batch the separate partial arrays with no gather, and a scalar reduction reads back once. **CPU gemm-like products** have no factor to amortize — break-even, so leave per-lane. Guard batched paths with `Nw == 1`.
