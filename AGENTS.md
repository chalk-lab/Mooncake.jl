...
- Batching the `N` per-lane calls into fewer wide ones pays only with a cost to amortize, and the payoff differs sharply by target. **GPU** wins broadly (launch-bound): `CUDA.CUBLAS.gemm_batched!`/`gemv_batched!` batch the separate partial arrays with no gather, and a scalar reduction reads back once. **CPU** is far narrower: stacking the `N` lane RHS of a shared-factor solve (triangular/LU/Cholesky) amortizes the factorization at the *solve* level, but the end-to-end frule win is eroded by the un-batchable per-lane product and the gather — erratic and shape-dependent (matrix RHS only; near break-even or a loss for the common vector RHS), so micro-bench before adopting, never a default. CPU gemm-like *products* have no factor to amortize — break-even, leave per-lane. Guard every batched path with `Nw == 1`.
- When a per-lane tangent applies the *same* factor in more than one solve, collapse them instead of batching: `op(A)⁻¹` is linear, so `α·op(A)⁻¹·dB − op(A)⁻¹·(α·op(dA)·X) = α·op(A)⁻¹·(dB − op(dA)·X)` — one solve of the combined RHS, not two (`trsm!`/`trtrs!`; uniform ~1.5–1.9× on CPU, every shape, no gather or guard).
