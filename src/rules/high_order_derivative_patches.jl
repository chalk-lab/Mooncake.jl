...
# `push!(::Stack, ::Tuple)` / `pop!(::Stack)` calls between the forward and
# pullback closures. Aliasing is preserved by the joint
# `zero_tangent((fwd_oc.captures, rvs_oc.captures))` call in
# `_compile_for_rule` / `_for_rule_cached_dual` — the IdDict inside
# `zero_tangent` produces shared `MutableTangent` slots for aliased primal
# `Stack` objects, so both closures' `Dual{Stack, MutableTangent}` slots
# point at the same tangent state. No special `dual_type`/lift on `Stack`
# is needed, and no named comms-stack primitives.
