...
# (`build_primitive_rrule`, chunk_size=DOF) rules via the nfwd engine, which
...
function build_primitive_rrule(sig::Type{<:Tuple{typeof(tanpi),<:IEEEFloat}})
    return NfwdMooncake.build_rrule(sig; chunk_size=1)
end
...
function build_primitive_rrule(sig::Type{<:Tuple{typeof(clamp),P,P,P}}) where {P<:IEEEFloat}
    ...
    return NfwdMooncake.build_rrule(sig; chunk_size=3)
end
