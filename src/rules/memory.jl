...
@inline function frule!!(
    ...
    # Struct-element MemoryRef: canonical V is `Dual{MemoryRef{P},
    # NTangent{...}}`. Set the primal once and write each lane's tangent
    # independently via `tangent(value, n)` (the per-lane Tangent built
    # by `_build_struct_tangent_dir`).
...
@inline function frule!!(
    ...
    ref::Mooncake.Lifted{MemoryRef{P},N,V_ref},
...
@inline function frule!!(
    ...
    ref::Mooncake.Lifted{MemoryRef{P},N,V_ref},
    size::Mooncake.Lifted{<:NTuple{M,Int}},
) where {N,P,M,V_ref<:Dual{MemoryRef{P},<:Mooncake.NTangent}}
    primal_size = primal(size)
    y = _new_(Array{P,M}, primal(ref), primal_size)
    tangents = ntuple(Val(N)) do lane
        _new_(Array{tangent_type(P),M}, Mooncake.tangent(ref, lane), primal_size)
    end
...
# `jl_genericmemory_copy` foreigncall — split by inner V.
# Wrapper-exception V (Dual{<:Memory, <:NTangent}): copy primal + each lane.
@inline function frule!!(
...
# Canonical NDual-element Memory V: single copy of the canonical container.
@inline function frule!!(
    ::Mooncake.Lifted{typeof(_foreigncall_),N},
    ::Mooncake.Lifted{Val{:jl_genericmemory_copy}},
    ::Mooncake.Lifted,
