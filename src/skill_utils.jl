...
function extract_meta(blks::Vector{CFGBlock})::StageMeta
    succs = _compute_cfg_successors(blks)
    return StageMeta(;
        block_count=length(blks),
        inst_count=sum(length(b.inst_ids) for b in blks),
