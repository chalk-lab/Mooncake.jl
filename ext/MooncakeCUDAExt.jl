...
Mooncake.@foldable fdata_type(::Type{CuArray{P,N,M}}) where {T<:IEEEFloat,P<:Complex{T},N,M} = CuArray{fdata_type(P),N,M}
...
function increment_internal!!(c::IncCache, x::CuArray{P,N,M}, y::CuArray{P,N,M}) where {P<:IEEEFloat,N,M}
    (x === y || haskey(c, x)) && return x
    c[x] = true
    x .+= y
    return x
end
function increment_internal!!(c::IncCache, x::CuArray{P,N,M}, y::CuArray{P,N,M}) where {T<:IEEEFloat,P<:Mooncake.Tangent{@NamedTuple{re::T, im::T}},N,M}
    (x === y || haskey(c, x)) && return x
    c[x] = true
    x_ = reinterpret(Complex{T}, x)
    y_ = reinterpret(Complex{T}, y)
...
function set_to_zero_internal!!(::Mooncake.SetToZeroCache, x::CuArray{Mooncake.Tangent{@NamedTuple{re::T, im::T}}, N, M}) where {T<:IEEEFloat, N, M}
    x_  = reinterpret(Complex{T}, x)
    x_ .= zero(Complex{T})
    return x
end
...
function _dot_internal(c::MaybeCache, x::CuArray{P}, y::CuArray{P}) where {T<:IEEEFloat,P<:Mooncake.Tangent{@NamedTuple{re::T, im::T}}}
    key = (x, y)
    haskey(c, key) && return c[key]::Float64
    x_ = reinterpret(Complex{T}, x)
    y_ = reinterpret(Complex{T}, y)
...
function _scale_internal(c::MaybeCache, x::Float64, y::CuArray{P, N, M}) where {T<:IEEEFloat,P<:Mooncake.Tangent{@NamedTuple{re::T, im::T}}, N, M}
...
Mooncake.@foldable tangent_type(::Type{CuArray{P,N,M}}, ::Type{NoRData}) where {T<:IEEEFloat,P<:Mooncake.Tangent{@NamedTuple{re::T, im::T}},N,M} = CuArray{P,N,M}
tangent(p::CuFloatArray, ::NoRData) = p
tangent(p::CuArray{P,N,M}, ::NoRData) where {T<:IEEEFloat,P<:Mooncake.Tangent{@NamedTuple{re::T, im::T}},N,M} = p
...
function rrule!!(
    ...
    _dims = map(primal, dims)
    return CoDual(P(undef, _dims), tangent_type(P)(undef, _dims)), NoPullback(p, init, dims...)
end
